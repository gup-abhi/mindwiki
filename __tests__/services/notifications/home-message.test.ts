...
describe('homeMessage', () => {
  ...
  it('acknowledges today on a long streak', () => {
    ...
    expect(homeMessage(ts, NOW)).toMatch(/today done — 8-day streak/i)
  })

  it('counts this week when written today on a short streak', () => {
    // Today + yesterday only → 2 active days this week, current streak 2.
    expect(homeMessage([daysAgo(0), daysAgo(1)], NOW)).toMatch(/today written — 2 days this week/i)
  })

  it('nudges gently after being away (broken streak, not today)', () => {
    // Last wrote 5 days ago → current streak 0, no entry today/yesterday.
    expect(homeMessage([daysAgo(5), daysAgo(6)], NOW)).toMatch(/been a little while/i)
  })

  it('encourages keeping a live streak going when not yet written today', () => {
    // Wrote each of the last 7 days but not today → streak alive (7), today open.
    const ts = Array.from({ length: 7 }, (_, i) => daysAgo(i + 1))
    expect(homeMessage(ts, NOW)).toMatch(/7-day streak going/i)
  })

  it('prompts a Day-N streak when short and not yet written today', () => {
    // Wrote yesterday and the day before, not today → streak 2.
    expect(homeMessage([daysAgo(1), daysAgo(2)], NOW)).toMatch(/^Day 2 —/)
  })
})
