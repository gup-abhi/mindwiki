...
export function useJournalEntry() {
  ...
  const submit = useCallback(async (): Promise<Result<SubmitOutcome>> => {
    ...
    try {
      ...
      // The durable save is the meaningful reflection boundary. Best-effort
      // notification bookkeeping never blocks the saved entry.
      void recordReflectionCompletion('journal', result.data.id, Date.now()).then(() =>
