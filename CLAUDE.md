...
│   │   └── subscription/          ← planned Phase 9 RevenueCat boundary
│   ├── native/
│   │   ├── LLMBridge.ts
│   │   └── CryptoModule.ts
│   ├── store/
...
│       └── push/                  ← future server relay; notifications are local-only today
│
...
yarn install
yarn expo prebuild             # generates ios/ and android/ when native builds are needed
cd ios && pod install && cd ..
cp .env.example .env.local   # add local values; never commit .env.local
...
EXPO_PUBLIC_API_URL=http://localhost:8787   # wrangler dev in local
```
