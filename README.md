...
- **Server** — Cloudflare Workers + R2 + KV (Wrangler); notifications are local-only today
- **Subscriptions** — RevenueCat (planned Phase 9; not yet wired)
...
src/
  ...
  services/       storage, llm, wiki, graph, sync, auth, crisis, notifications, digest
  native/         LLMBridge, CryptoModule
  store/          Zustand stores
server/           Cloudflare Workers backend (auth, encrypted storage)
                  subscription/ is planned for Phase 9
                  ios/ and android/ are generated by Expo prebuild when needed
...
```bash
yarn install
# Generate native projects before installing pods or running native builds.
yarn expo prebuild
